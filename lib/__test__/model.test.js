...
describe('Model class', () => {
  ...
  // this test is an intersection of a lot of parts of code
  // dependencies are a little heavy
  describe('#new(fields)', () => {
    const model = new Model('Opportunity', {}, { proxyHandler: { writables: {} } })
    it('returns a grunt', () => {
      const expected = Grunt
      const actual = model.new()

      expect(actual).toBeInstanceOf(expected)
    })

    it('passes all fields to grunt', () => {
      const input = {
        field: true,
        field2: true,
        anotherField: true
