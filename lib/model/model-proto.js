...
const modelProto = {
  ...
  getRelField (object) {
    return this.__schema.getRel(object)
  },

  fetchAssoc (docs) {
    const ids = docs.map(doc => doc.id)
    const [ withField, modelName ] = this._with.entries().next().value
    // getModel exists on prototype chain on arbiter
    const model = this.getModel(modelName)
    const relField = model.getRelField(this.__schema.sfObject)
    const where = {}
    where[relField] = ids
    model.where(where)
    return model.exec()
      .then(withDocs => {
        const ids = withDocs.reduce((obj, withDoc) => {
          const withId = withDoc[relField]
          obj[withId] = obj[withId]
            ? obj[withId].push(withDoc)
            : [ withDoc ]
        }, {})
        docs.forEach(doc => {
          doc[withField] = ids[doc.id]
        })
        return docs
      })
  },

  checkForSingleReturn (docs) {
  ...
  with (assoc) {
    validate.checkType(assoc, 'string', `${this.__name}.with(assoc)`)
    const withModel = this.__schema.getAssoc(assoc)
    if (!withModel) {
      throw new Error(`${this.__name} does not have assoc with ${assoc}`)
    }
  ...
  /**
   * Given a collection of Salesforce query results this will loop through them and create doc instances for each one. If results is a single value then it will get turned into doc and returned
   *
   * @param {array} results
   * @returns {array} docs
   */
    ...
    return this.getConnection()
      ...
      .then(queryResult => {
        ...
        if (idClause && !Array.isArray(idClause)) {
          return [ queryResult.shift() ].map(this.mapResult, this)
        }
      ...
      .then(docs => {
        if (this._with.size) {
          return this.fetchAssoc(docs)
            .then(docs => {
              docs = this.checkForSingleReturn(docs)
              this.clearState()
              return docs
            })
        }
