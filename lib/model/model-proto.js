...
const modelProto = {
  ...
  findById (id) {
    this._fields = this.schema.getAllFields()
    this.where({ id: id })
    return this
  },
  ...
  fields (...args) {
    ...
    this._fields = this.schema.expandFields(args)
    return this
  },
  ...
  doc (results) {
    ...
    return Object.assign(instance, results, { changeset: {} })
  },
  ...
  buildSelect () {
    if (this._mappings.length === 0) {
      this._mappings = this.schema.mapFields(this._fields)
    }
  ...
  /**
   * Takes the fields that a model is going to query and builds up a skeleton object of the results.
   *
  ...
  getResultSkeleton () {
    ...
    this._fields.forEach(field => {
      if (~field.search(/\./)) {
        dotPath.set(result, field, undefined)
      } else {
  ...
  mapResult (queryResult) {
    ...
    this._fields.forEach((field, idx) => {
      const queryValue = dotPath.get(queryResult, this._mappings[idx])
