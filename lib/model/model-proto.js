...
const modelProto = {
  ...
  find (opts) {
    if (opts) {
      gandalf.checkType(opts, 'Object', `${this.__name}.find(opts)`)
      this.where(opts)
    }
    return this
  },
   ...
   */
  findByIds (ids) {
    gandalf.checkType(ids, 'Array', `${this.__name}.findByIds(ids)`)
    this.where({ id: ids })
    return this
  },
   ...
   */
  where (opts) {
    gandalf.checkType(opts, 'Object', `${this.__name}.where(opts)`)
  ...
  inject (query, params = {}, quotes = true) {
    gandalf.checkType(query, 'string', `${this.__name}.inject(query, .., ..)`)
    gandalf.checkType(params, 'Object', `${this.__name}.inject(.., params, ..)`)
   ...
   */
  query (query) {
    return gandalf.async.checkType(query, 'string', `${this.__name}.query(query)`)
  },
   ...
   */
  RAW (query) {
    return gandalf.async.checkType(query, 'string', `${this.__name}.RAW(query)`)
