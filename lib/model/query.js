...
class Query {
   ...
   */
  offset (num) {
    valid.checkType(num, 'number', `${this.model.name}.offset(num)`)
    this._offset = num
  ...
  /**
   * Adds an `Order by` object to query state
   *
   * @param string num
   * @chainable
   * @returns Query
  ...
  orderBy (field, dir = 'asc') {
    valid.checkType(field, 'string', `${this.model.name}.orderBy(field)`)
    if (dir !== 'desc' && dir !== 'asc') {
      dir = 'asc'
    }
    this._orderBy = {
      dir,
      value: this.schema.mapFields([field])[0]
    }
  ...
  /**
   * return a string with the Soql query
   *
   * @returns string
   *
   * @memberof Query
  ...
  buildQuery () {
    ...
    return [
      this.buildSelect(),
      `FROM ${this.schema.sfObject}`,
      this.buildWhere(),
      this.buildOffset(),
      this.buildLimit(),
      this.buildOrderBy()
    ].reduce((queryString, chunck) => {
      if (chunck) {
        queryString += `${chunck} `
      }
      return queryString
  ...
  /**
   * Builds offset portion of query string
   *
   * @returns string
   *
   * @memberof Query
  ...
  /**
   * Builds limit portion of query string
   *
   * @returns string
   *
   * @memberof Query
  ...
  /**
   * Builds order by portion of query string
   *
   * @returns string
   *
   * @memberof Query
  ...
  buildOrderBy () {
    ...
    return `ORDER BY ${this._orderBy.value} ${this._orderBy.dir}`
  }
