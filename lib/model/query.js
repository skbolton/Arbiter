...
class Query {
  ...
  with (assoc) {
    if (valid.getType(assoc) === 'string') {
      ...
      if (!assocModel) {
        throw new Error(
          `${this.model.name} does not have association with key of ${assoc}`
        )
      }
  ...
  exec () {
    ...
    return this.model
      .getConnection()
      .then(conn => conn.query(queryString))
      .then(({ records }) => {
        // delete sf metadata
      ...
      .then(grunts => {
        ...
        return this._with.size ? this.fetchAssocs(grunts) : grunts
      })
  ...
  fetchAssocs (grunts) {
    ...
    if (!relField) {
      const error = new Error(
        `Model ${assocModel.name} does not have a relation field to ${
          this.model.name
        } defined. Cannot fetch associated ${assocField}`
      )
  ...
  buildWhere () {
    ...
    mappedFields.forEach((field, idx) => {
      ...
      if (type === 'Array') {
        whereClauses.push(`${field} IN ${transform.soqlArrayStringify(value)}`)
      }
  ...
  static inject (query, params = {}, quotes = true) {
    return Object.keys(params).reduce((newQuery, key) => {
      ...
      const replacement = quotes ? `'${params[key]}'` : params[key]
  ...
  static complexWhere (field, opts) {
    ...
    Object.keys(opts).forEach(option => {
      ...
      if (!transformer) {
        throw new Error(
          `Cannot build where clause for field: ${field} using option: ${option}. Options must be one of ${transform.comparitors.join(
            ', '
          )}`
        )
      }
