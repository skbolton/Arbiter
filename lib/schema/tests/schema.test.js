...
describe('Schema Class', () => {
  ...
  describe('#addChildSchema(schema, path)', () => {
    it('should throw if `schema` is not a Schema instance', () => {
      const shouldThrow = () => {
        oppSchema.addChildSchema('not a schema', 'somepath')
      }
    ...
    it('should throw if `path` does not already exist in schema', () => {
      const shouldThrow = () => {
        oppSchema.addChildSchema(lineItemsSchema, 'nope.bad.bad')
      }

      const shouldThrowAgain = () => {
        // almost good path
        oppSchema.addChildSchema(lineItemsSchema, 'project.bad.proposalCAD')
      }
    ...
    it('should add `schema` to tree at path', () => {
      ...
      // now add it
      oppSchema.addChildSchema(lineItemsSchema, path)
